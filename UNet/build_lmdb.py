...
def generate_database(img_list, database_name, image_filepath, mask_filepath, output_folder, tile_size):
    ...
    image_env = lmdb.open(output_image_lmdb_file, map_size=int(1e10))
    ...
    for i in range(len(img_list)):
        ...
        if tile_size > 0:
            ...
            txn_nb += 1
            # if(np.max(msk) > 1):
            #     raise RuntimeError('larger than 1')
            write_img_to_db(image_txn, img, msk, key_str)
...
def main(image_folder, test_image_folder, mask_folder, test_mask_folder, output_folder, dataset_name, train_fraction, image_format, use_tiling, tile_size):
    ...
    test_image_folder = os.path.abspath(test_image_folder)
    mask_folder = os.path.abspath(mask_folder)
    test_mask_folder = os.path.abspath(test_mask_folder)
    ...
    img_files_test = [f for f in os.listdir(test_mask_folder) if f.endswith('.{}'.format(image_format))]
    if len(img_files) == 0:
    ...
    generate_database(test_img_files, database_name, test_image_folder, test_mask_folder, output_folder, tile_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--test_image_folder')
    ...
    parser.add_argument('--test_mask_folder')
    ...
    parser.add_argument('--data', dest='data', type=str, help='name of the dataset to be used in creating the lmdb files', default='HES')
    ...
    # main is to build lmdb database!!!
    main(image_folder, args.test_image_folder, mask_folder, args.test_mask_folder, output_folder, dataset_name, train_fraction, image_format, use_tiling, tile_size)
